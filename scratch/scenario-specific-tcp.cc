...
NS_LOG_COMPONENT_DEFINE ("ScenarioSpecificTcp");

class ScenarioSpecificTcp
{
public:
  ScenarioSpecificTcp ();
...
private:
  ...
  std::list <Ptr<BulkSendApplication> > m_srcApps;
...
ScenarioSpecificTcp::ScenarioSpecificTcp ()
{
}

void
ScenarioSpecificTcp::CheckThroughput ()
{
...
void
ScenarioSpecificTcp::CaseRun (uint32_t nWifis, uint32_t nSinks, uint32_t nSources, double totalTime, std::string rate,
                           ...
                           std::string animationFileName, std::string clickFile)
{
  ...
  Simulator::Schedule (Seconds(m_totalTime),&ScenarioSpecificTcp::CheckThroughput,this);
...
void
ScenarioSpecificTcp::CreateFile ()
{
...
void
ScenarioSpecificTcp::CreateNodes ()
{
...
void
ScenarioSpecificTcp::SetupMobility ()
{
...
void
ScenarioSpecificTcp::CreateDevices ()
{
...
void
ScenarioSpecificTcp::InstallInternetStack ()
{
...
void
ScenarioSpecificTcp::InstallApplications ()
{
  ...
  for (uint32_t i=0;i<m_nSinks;i++)
  {
      ...
      Ptr<BulkSendApplication> packetSrcApp = CreateObjectWithAttributes<BulkSendApplication>(
          "Remote", AddressValue (InetSocketAddress (interfaces.GetAddress (sink), 10)),
          "SendSize", UintegerValue (1500),
          "MaxBytes", UintegerValue (2000000000),
          "StartTime", TimeValue(Seconds (m_dataStart + start)),
          "StopTime", TimeValue(Seconds(m_dataStart + start + duration))
          );
      nodes.Get (source)->AddApplication (packetSrcApp);

      m_srcApps.push_back (packetSrcApp);
