...
NS_LOG_COMPONENT_DEFINE ("StatsVideoTxop");

class StatsVideoTxop
{
public:
  StatsVideoTxop ();
...
main(int argc, char** argv)
{
  ...
  double totalTime = 144.0;
  double dataStart = 4.0;

  uint32_t seedNumber = 1;
  uint32_t runNumber = 1;

  std::string rate ("25Mbps");
  std::string constRate ("OfdmRate6Mbps");
  ...
  cmd.AddValue ("constRate", "Constant rate wifi rate [Default:OfdmRate6Mbps]", constRate);
  cmd.AddValue ("packetSize", "CBR packet size in bytes[Default:160]", packetSize);
  ...
  test = StatsVideoTxop ();
  test.CaseRun (nWifis, nSinks, nSources, totalTime, rate, constRate, dataStart, CSVfileName, topoFileName, pairFileName, startFileName, durationFileName, animationFileName);

  return 0;
}

StatsVideoTxop::StatsVideoTxop ()
{
}

void
StatsVideoTxop::CheckThroughput ()
{
...
void
StatsVideoTxop::CaseRun (uint32_t nWifis, uint32_t nSinks, uint32_t nSources, double totalTime, std::string rate, std::string constRate,
                           ...
                           std::string animationFileName)
{
  ...
  Simulator::Schedule (Seconds(m_totalTime),&StatsVideoTxop::CheckThroughput,this);
...
StatsVideoTxop::CreateDevices ()
{
  ...
  wifi.SetRemoteStationManager ("ns3::ConstantRateWifiManager", "DataMode", StringValue (m_constRate), "ControlMode", StringValue ("OfdmRate6Mbps"));
  devices = wifi.Install (wifiPhy, wifiMac, nodes);

  wifiPhy.EnablePcap (("pcap-" + m_CSVfileName).c_str(),nodes,false);
}

void
StatsVideoTxop::InstallInternetStack ()
{
  ClickInternetStackHelper click;
  click.SetClickFile (nodes, "nsclick-dsdv-pett-video.click");
...
void
StatsVideoTxop::InstallApplications ()
{
  ...
  for (uint32_t i=0;i<m_nSinks;i++)
  {
      ...
      NS_LOG_UNCOND ("Setting start as " << start << " + " << m_dataStart);
      std::getline ( durationFile, tmpTime);
      double duration = atof (tmpTime.c_str());
      NS_LOG_UNCOND ("Setting duration as " << duration);
      ...
      // don't add a source to the first node, which we're testing
      if (i!=0)
        {
